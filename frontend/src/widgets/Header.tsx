...
import { confirmDialog } from "../shared/ui/notifications";
...
export function Header({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      const target = event.target;
      if (!(target instanceof Node)) return;
      if (desktopNotificationsRef.current?.contains(target)) return;
      if (mobileNotificationsRef.current?.contains(target)) return;
              ...
              {isAuthenticated && (
                <div ref={desktopNotificationsRef} className="relative">
                  <button
                    ...
                    data-testid="header-notifications-toggle-desktop"
                  ...
                  {showNotifications && (
                    <div
                      data-testid="header-notifications-panel-desktop"
            ...
            {isAuthenticated ? (
              <div ref={mobileNotificationsRef} className="relative">
                <button
                  type="button"
                  onClick={handleToggleNotifications}
                  data-testid="header-notifications-toggle-mobile"
                ...
                {showNotifications && (
                  <div
                    data-testid="header-notifications-panel-mobile"
                    className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg"
                  >
                    <div className="flex items-center justify-between gap-2 border-b p-4">
                      <div className="font-semibold">Уведомления</div>
                      {notifications.length > 0 ? (
