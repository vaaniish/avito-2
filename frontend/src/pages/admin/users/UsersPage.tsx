...
export function UsersPage() {
  ...
  const loadUsers = async () => {
    try {
      const result = await apiGet<AdminUser[]>(`/admin/users?_=${Date.now()}`);
      setUsers(result);
      setSelectedUser((prev) => {
        if (!prev) return result[0]?.id ?? null;
        return result.some((user) => user.id === prev) ? prev : result[0]?.id ?? null;
      });
    } catch (error) {
  ...
  const updateUserRole = async (
    ...
    if (role === "regular") {
      const confirmed = await confirmDialog({
        title: "Снять партнёрство",
        description: `Пользователь ${user.name} будет переведён обратно в покупатели.`,
        confirmLabel: "Снять партнёрство",
        cancelLabel: "Отмена",
        confirmTone: "danger",
      });
      if (!confirmed) return;
    ...
    try {
      const response = await apiPatch<{ success: boolean; role: "regular" | "partner" }>(
      ...
      notifySuccess(
        role === "partner" ? "Пользователь повышен до продавца." : "Партнёрство снято.",
      );
      await loadUsers();
    } catch (error) {
      notifyError(error instanceof Error ? error.message : "Не удалось обновить роль пользователя");
    } finally {
              ...
              {selectedUserData.role !== "admin" && (
                <button
                  ...
                  onClick={(event) =>
                    void updateUserRole(
                      selectedUserData,
                      selectedUserData.role === "partner" ? "regular" : "partner",
                      event,
                    )
                  }
                  className="btn-secondary w-full py-2"
                  disabled={pendingRoleUserId === selectedUserData.id}
                >
                  {pendingRoleUserId === selectedUserData.id
                    ? "Обновляем роль..."
                    : selectedUserData.role === "partner"
                      ? "Снять партнёрство"
                      : "Сделать продавцом"}
                </button>
              )}
