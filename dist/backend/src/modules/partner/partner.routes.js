...
const listing_moderation_1 = require("./listing-moderation");
const order_delivery_1 = require("./order-delivery");
...
const order_status_fsm_1 = require("../orders/order-status-fsm");
...
const DEFAULT_TRACKING_PROVIDER = "yandex_pvz";
...
function parseSellerEditableOrderStatus(value) {
    const raw = parseOrderStatus(value);
    if (raw === "PREPARED")
        return raw;
    return null;
}
function normalizeTrackingProvider(value) {
...
function mapExternalDeliveryStatusToOrderStatus(status) {
...
function parsePayoutLegalType(value) {
    if (value === "COMPANY")
        return "COMPANY";
    if (value === "IP")
        return "IP";
...
function shouldSyncDeliveryStatus(order) {
...
async function hasBlockingOrderForListing(listingId) {
    const linked = await prisma_1.prisma.marketOrderItem.findFirst({
        where: {
...
async function syncSingleOrderDeliveryStatus(order) {
    if (!shouldSyncDeliveryStatus(order)) {
        return;
    }
    const provider = normalizeTrackingProvider(order.tracking_provider);
    const tracking = await (0, order_delivery_1.fetchTrackingStatus)({
        provider,
    ...
    if (nextStatus && nextStatus !== order.status) {
        if ((0, order_status_fsm_1.isOrderStatusTransitionAllowed)({
...
function parseListingStatus(value) {
    if (value === "active")
        return LISTING_ACTIVE;
    if (value === "inactive")
        return LISTING_INACTIVE;
    if (value === "moderation")
        return LISTING_MODERATION;
    return null;
}
function resolveSellerStatusTransition(current, requested) {
...
function formatListingPublicId(id) {
...
async function loadSellerModerationContext(sellerId) {
    const seller = await prisma_1.prisma.appUser.findUnique({
        where: { id: sellerId },
        select: {
            joined_at: true,
            seller_profile: {
...
async function resolveAutoModerationDecision(params) {
    ...
    return (0, listing_moderation_1.evaluateListingModeration)({
        title: params.title,
...
async function resolveCatalogItemId(type, rawCategory) {
    ...
    if (!categoryName || categoryName === "No category")
        return null;
    const itemByName = await prisma_1.prisma.catalogItem.findFirst({
        where: {
            name: {
                equals: categoryName,
                mode: "insensitive",
    ...
    const categoryByName = await prisma_1.prisma.catalogCategory.findFirst({
        where: {
            type,
            name: {
                equals: categoryName,
    ...
    const subcategoryByName = await prisma_1.prisma.catalogSubcategory.findFirst({
        where: {
            name: {
                equals: categoryName,
                mode: "insensitive",
...
partnerRouter.get("/listings/title-suggestions", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        const [listingTitles, catalogTitles] = await Promise.all([
            ...
            prisma_1.prisma.catalogItem.findMany({
                where: {
                    name: {
                        contains: query,
                        mode: "insensitive",
        ...
        const scoreTitle = (title, baseScore) => {
...
partnerRouter.get("/payout-profile", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        const profile = await prisma_1.prisma.sellerPayoutProfile.findUnique({
            where: {
                seller_id: session.user.id,
            },
            select: {
...
partnerRouter.put("/payout-profile", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        if (!legalName ||
            ...
            res.status(400).json({
                error: "Invalid payout requisites. Check legal name, tax id, account, BIC and correspondent account.",
            });
            return;
        }
        const payload = {
        ...
        const saved = await prisma_1.prisma.sellerPayoutProfile.upsert({
            where: {
                seller_id: session.user.id,
            },
            create: {
...
partnerRouter.get("/listings/category-guess", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        const pushScore = (category, score, source) => {
            if (!category)
                return;
        ...
        for (const row of listingMatches) {
            ...
            if (rowTitle === normalizedTitle)
                score += 36;
            else if (rowTitle.startsWith(normalizedTitle))
                score += 20;
            else if (rowTitle.includes(normalizedTitle))
                score += 10;
        ...
        for (const row of catalogMatches) {
            ...
            if (itemName === normalizedTitle)
                score += 22;
            else if (itemName.startsWith(normalizedTitle))
                score += 14;
            else if (itemName.includes(normalizedTitle))
                score += 7;
...
partnerRouter.post("/listings", async (req, res) => {
    try {
        ...
        const category = typeof body.category === "string" ? body.category.trim() : "No category";
...
partnerRouter.patch("/listings/:publicId", async (req, res) => {
    try {
        ...
        const nextImagesFromBody = body.images === undefined ? undefined : normalizeImageArray(body.images);
        const legacyImage = typeof body.image === "string" ? body.image.trim() : undefined;
        const nextImages = nextImagesFromBody !== undefined
            ? nextImagesFromBody
        ...
        const nextAttributes = body.attributes === undefined ? undefined : normalizeAttributes(body.attributes);
        ...
        const nextCategoryForModeration = nextCategory === undefined
...
partnerRouter.post("/listings/:publicId/toggle-status", async (req, res) => {
    try {
        ...
        const requestedStatus = currentStatus === LISTING_INACTIVE
            ? LISTING_MODERATION
            : LISTING_INACTIVE;
        ...
        if (!transition) {
            res.status(409).json({
                ...
                status: (0, format_1.toPartnerListingStatus)(existing.status),
            });
            return;
        }
        if (transition.nextStatus === LISTING_MODERATION &&
            ...
            res.status(409).json({
                error: "Нельзя повторно активировать объявление: по нему уже есть неотмененный заказ.",
                status: (0, format_1.toPartnerListingStatus)(existing.status),
            });
            return;
        }
...
partnerRouter.patch("/listings/:publicId/status", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        const transition = resolveSellerStatusTransition(existing.status, nextStatus);
        if (!transition) {
            res.status(409).json({
                error: "Unsupported status transition",
                status: (0, format_1.toPartnerListingStatus)(existing.status),
...
partnerRouter.delete("/listings/:publicId", async (req, res) => {
    try {
        ...
        if (await hasBlockingOrderForListing(existing.id)) {
            res.status(409).json({
                error: "Нельзя удалить объявление, связанное с неотмененным заказом. Это нарушит финансовую прозрачность.",
            });
            return;
        }
...
partnerRouter.patch("/orders/:publicId/status", async (req, res) => {
    try {
        ...
        (0, order_status_fsm_1.assertOrderStatusTransitionAllowed)({
            fromStatus: existing.status,
            toStatus: nextStatus,
            context: "seller.mark_prepared",
        });
        const updatedCount = await prisma_1.prisma.marketOrder.updateMany({
...
partnerRouter.patch("/orders/:publicId/tracking", async (req, res) => {
    try {
        const session = await (0, session_1.requireAnyRole)(req, [ROLE_SELLER, ROLE_ADMIN]);
        if (!session.ok) {
            res.status(session.status).json({ error: session.message });
        ...
        const validation = await (0, order_delivery_1.validateTrackingNumber)({
            provider,
        ...
        const refreshed = await prisma_1.prisma.marketOrder.findUnique({
            where: { id: existing.id },
            select: {
                id: true,
                public_id: true,
        ...
        const finalState = await prisma_1.prisma.marketOrder.findUnique({
            where: { id: existing.id },
            select: {
                status: true,
                tracking_provider: true,
...
partnerRouter.post("/questions/:publicId/answer", async (req, res) => {
    try {
        ...
        const circumventionSignals = (0, anti_circumvention_1.detectCircumventionSignals)(answer);
        if (circumventionSignals.length > 0) {
            ...
            res.status(400).json({
                error: "Ответ отклонен: запрещено передавать контакты и уводить сделку вне платформы. Нарушение зафиксировано.",
