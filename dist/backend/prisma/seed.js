...
async function main() {
    ...
    if (!productCategory || !serviceCategory || !homeCategory)
    ...
    const itemIdByTypeAndName = new Map();
    ...
    const listingId = (publicId) => {
        const listing = listingByPublic.get(publicId);
        if (!listing)
            throw new Error(`Listing ${publicId} not found`);
        return listing.id;
    };
