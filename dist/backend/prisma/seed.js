...
async function main() {
    ...
    const orderTotalByPublic = new Map();
    for (const order of ordersSeed) {
        for (const item of order.items) {
            ...
            if (listingSeed.seller_public_id !== order.seller_public_id) {
                throw new Error(`Order ${order.public_id} seller mismatch for listing ${item.listing_public_id}: expected ${listingSeed.seller_public_id}, got ${order.seller_public_id}`);
            }
        }
    ...
    const achievementRules = {
        ...
        "Секрет: Теневая сделка": { metric: "max_deal", target: 250000 },
    };
    const sandboxSales = [
    ...
    for (const sale of sandboxSales) {
        ...
        for (const [achievementName, rule] of Object.entries(achievementRules)) {
            ...
            const metricValue = rule.metric === "orders"
                ? ordersCount
    ...
    const [usersCount, addressesCount, categoriesCount, subcategoriesCount, itemsCount, listingsCount, reviewsCount, questionsCount, wishlistCount, marketOrdersCount, marketItemsCount, transactionsCount, complaintsCount, kycCount, tiersCount, logsCount, partnershipCount, partnersCount, levelsCount, achievementsCount, sandboxOrdersCount, xpCount, partnerAchievementsCount,] = await Promise.all([
    ...
    console.log(`Users: ${usersCount}, addresses: ${addressesCount}, categories: ${categoriesCount}/${subcategoriesCount}/${itemsCount}`);
    console.log(`Listings: ${listingsCount}, reviews: ${reviewsCount}, questions: ${questionsCount}, wishlist: ${wishlistCount}`);
    console.log(`Orders: ${marketOrdersCount} (${marketItemsCount} items), transactions: ${transactionsCount}, complaints: ${complaintsCount}`);
    console.log(`KYC: ${kycCount}, tiers: ${tiersCount}, audit logs: ${logsCount}, partnership requests: ${partnershipCount}`);
    console.log(`Gamification -> partners: ${partnersCount}, levels: ${levelsCount}, achievements: ${achievementsCount}, sandbox orders: ${sandboxOrdersCount}, xp accruals: ${xpCount}, unlocked achievements: ${partnerAchievementsCount}`);
