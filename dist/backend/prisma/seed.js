...
async function main() {
    ...
    const policyMap = new Map((await prisma.platformPolicy.findMany({ select: { id: true, public_id: true } })).map((policy) => [policy.public_id, policy.id]));
    ...
    await prisma.sellerPayoutProfile.createMany({
        data: [
            ...
            rejection_reason: p[10] === "REJECTED" ? "Не прошла проверка реквизитов" : null,
        })),
    });
