...
function getCurrentLevel(levels: LoyaltyLevel[], xp: number): LoyaltyLevel {
  if (levels.length === 0) {
...
function getMetricValue(metrics: AchievementMetrics, metric: AchievementMetric): number {
  return metrics[metric];
}

const gamificationRouter = Router();

gamificationRouter.get("/partner-stats", async (_req: Request, res: Response) => {
  try {
    ...
    const unlockedById = new Map(
      partner.partner_achievements.map((partnerAchievement) => [
...
gamificationRouter.post("/simulate-sale", async (req: Request, res: Response) => {
  try {
    const rawDealAmount = (req.body as { deal_amount?: unknown })?.deal_amount;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (totalPenaltyXp > 0) {
        ...
        if (operationalFeeXp > 0) penaltyParts.push(`операционный сбор ${operationalFeeXp} XP`);
        if (lowCheckPenaltyXp > 0) penaltyParts.push(`мелкая сделка ${lowCheckPenaltyXp} XP`);
        if (lowRatingPenaltyXp > 0) penaltyParts.push(`низкий рейтинг ${lowRatingPenaltyXp} XP`);
...
gamificationRouter.post("/reset-sandbox", async (_req: Request, res: Response) => {
  try {
