...
const MIN_DEAL_AMOUNT = 10_000;
const MAX_DEAL_AMOUNT = 100_000;
const MIN_MANUAL_DEAL_AMOUNT = 100;
const MAX_MANUAL_DEAL_AMOUNT = 10_000_000;
...
function calculateCommissionRate(xpThreshold: number): number {
  if (xpThreshold >= 15_000) return 5.0;
  if (xpThreshold >= 7_000) return 6.0;
  if (xpThreshold >= 3_500) return 7.0;
  if (xpThreshold >= 1_500) return 8.0;
  if (xpThreshold >= 500) return 9.0;
  return 10.0;
}

function getRatingMultiplier(rating: number): number {
  if (rating >= 4.7) return 1.2;
  if (rating <= 3.0) return 0.6;
  return 1.0;
}

function getXpRuleDescription(xpCoefficient: number): string {
  if (xpCoefficient <= 1) {
...
function getCurrentLevel(
  ...
  return levels.reduce(
    (resolved: LoyaltyLevel, level: LoyaltyLevel) => {
...
gamificationRouter.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      const [partner, levels, achievements, orderCount, salesAgg, maxDealAgg] =
        await Promise.all([
      ...
      const nextLevel =
        levels.find(
          (level: LoyaltyLevel) =>
            level.xp_threshold > partner.current_xp,
        ) ?? null;
      ...
      const unlockedById = new Map(
        partner.partner_achievements.map(
          (
            partnerAchievement: PartnerAchievement & { achievement: Achievement },
          ) => [partnerAchievement.achievement.id, partnerAchievement.achieved_date],
        ),
      );

      const achievementsCatalog = achievements.map((achievement: Achievement) => {
        ...
        const progressPercent = Math.min(
          Math.round((progressValue / rule.target) * 100),
          100,
        );
        ...
        return {
          ...
          progress_label: `${progressValue.toLocaleString("ru-RU")} / ${rule.target.toLocaleString(
            "ru-RU",
          )}`,
      ...
      res.json({
        ...
        loyalty_levels: levels.map((level: LoyaltyLevel) => ({
          level_id: level.id,
        ...
        xp_policy: {
          formula:
            "XP = (оборот / 100) * коэффициент уровня * коэффициент рейтинга",
          deductions: [
        ...
        achievements: partner.partner_achievements.map(
          (
            partnerAchievement: PartnerAchievement & { achievement: Achievement },
          ) => ({
        ...
        next_level: nextLevel
          ? {
              ...
              xp_needed: Math.max(
                nextLevel.xp_threshold - partner.current_xp,
                0,
              ),
            }
          : null,
...
gamificationRouter.get("/xp-history", async (_req: Request, res: Response) => {
  try {
    const xpHistory = await prisma.xpAccrual.findMany({
      where: {
        order: {
    ...
    res.json(
      xpHistory.map((item: XpAccrual & { order: Order }) => ({
        accrual_id: item.id,
...
gamificationRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const rawDealAmount = (req.body as { deal_amount?: unknown })
        ?.deal_amount;

      let dealAmount: number;
      if (
        rawDealAmount === undefined ||
        rawDealAmount === null ||
        rawDealAmount === ""
      ) {
        dealAmount = randomInt(MIN_DEAL_AMOUNT, MAX_DEAL_AMOUNT + 1);
      } else {
      ...
      const result = await prisma.$transaction(async () => {
        ...
        const rawSaleXp = Math.round(
          (dealAmount / 100) *
            previousLevel.xp_coefficient *
            ratingMultiplier,
        );
        const operationalFeeXp = Math.floor(rawSaleXp * 0.1);
        const lowCheckPenaltyXp = dealAmount <= 1_000 ? 1 : 0;
        const lowRatingPenaltyXp =
          partner.rating < 4.5 ? Math.ceil(rawSaleXp * 0.15) : 0;
        const totalPenaltyXp =
          operationalFeeXp + lowCheckPenaltyXp + lowRatingPenaltyXp;
        ...
        const [
          existingPartnerAchievements,
          achievements,
          totalOrders,
          totalSales,
          maxDeal,
        ] = await Promise.all([
          prisma.partnerAchievement.findMany({
            where: { partner_id: SANDBOX_PARTNER_ID },
            select: { achievement_id: true },
          }),
          prisma.achievement.findMany({ orderBy: { id: "asc" } }),
          prisma.order.count({ where: { partner_id: SANDBOX_PARTNER_ID } }),
          prisma.order.aggregate({
            where: { partner_id: SANDBOX_PARTNER_ID },
            _sum: { deal_amount: true },
          }),
        ...
        const unlockedAchievementIds = new Set(
          existingPartnerAchievements.map(
            (item: { achievement_id: number }) => item.achievement_id,
          ),
        );
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "Sandbox reset complete: XP, orders and achievements were cleared.",
      });
    } catch (error) {
