...
export type FinanceOrderStatus =
  ...
  | "COMPLETED"
  | "CANCELLED";
export type FinanceSettlementBucketKey =
  | "pendingPayment"
  | "inProgress"
  | "readyToPayout"
  | "problem";
...
export const FINANCE_TRANSACTION_LABELS: Record<
...
export const FINANCE_SETTLEMENT_BUCKETS: Record<
  ...
  Omit<
    ...
    "count" | "amount" | "commissions" | "sellerPayout"
  >
> = {
  pendingPayment: {
    key: "pendingPayment",
...
export function parseFinanceDateRangeInput(input: {
  from?: unknown;
  to?: unknown;
  groupBy?: unknown;
}): {
  ...
  return {
    ...
    groupBy: parseFinanceGroupBy(input.groupBy),
  };
}

export function parseFinanceTransactionStatus(
  value: unknown,
): FinanceTransactionStatus | null {
  if (typeof value !== "string" || value === "all") return null;
  const normalized = value.toUpperCase();
  return FINANCE_TRANSACTION_STATUSES.includes(
...
export function parseFinanceOrderStatus(
  value: unknown,
): FinanceOrderStatus | null {
  if (typeof value !== "string" || value === "all") return null;
  const normalized = value.toUpperCase();
...
export function parseFinanceReportLimit(value: unknown): number {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0) {
    return FINANCE_REPORT_DEFAULT_LIMIT;
  }
...
export function isFinanceEarnedStatus(
